...
#'
#' @param plot_title Character. If specified, adds the specified text as a title on the figure. If NULL, no plot title included.
#'
...
#' plotPhotoCover(site = "CALISL", palette = "default", species = spp)
#'
#' # Return only the top 3 species for barnacle photo plots
#' plotPhotoCover(site = "SHIHAR", palette = "default", community = "Barnacle", top_spp = 3)
#'
...
plotPhotoCover <- function(park = "all", site = "all", plotName = "all",
  ...
  cols = c("ALGBRO" = "#A4755B", "ALGGRE" = "#C4E133", "ALGRED" = "#FF4C53", "ARTCOR" = "#D78AAE",
           ...
           "KELP"   = "#4DA551", "MUSSPP" = "#6F88BF", "NOSAMP" = "black",
           "OTHINV" = "#F59617", "OTHPLA" = "magenta","OTHSUB" = "#8A838A",
  ...
  shps = c("ALGBRO" = 21, "ALGGRE" = 23, "ALGRED" = 24, "ARTCOR" = 25,
           ...
           "KELP"   = 21, "MUSSPP" = 23, "NOSAMP" = 21,
           "OTHINV" = 24, "OTHPLA" = 23, "OTHSUB" = 25,
  ...
  sz = c("ALGBRO" = 4, "ALGGRE" = 3.5, "ALGRED" = 3, "ARTCOR" = 3,
         ...
         "KELP"   = 4, "MUSSPP" = 3.5, "NOSAMP" = 4,
         "OTHINV" = 3, "OTHPLA" = 3.5, "OTHSUB" = 3,
         "PALPAL" = 4, "PORSPP" = 3.5, "ULVENT" = 3, "ULVINT" = 3,
         "ULVLAC" = 4, "UNIDEN" = 3.5, "BOLT"   = 3, "ROCK"   = 3,
         "WATER"  = 4, "SAND" = 3.5, "TAR" = 3, "REDGRP" = 2)

  labels = c("ALGBRO" = "Algae - Brown",
             "ALGGRE" = "Algae - Green", "ALGRED" = "Algae - Red",
             "ARTCOR" = "Articulated Corallines",
             "ASCEPI" = "Ascophyllum epibiont",
             "ASCNOD" = "A. nodosum (Knotted wrack)",
             "CHOMAS" = "Irish moss",
             "CRUCOR" = "Crustose coraline",
             "NONCOR" = "Crustose non-coraline",
             "BARSPP" = "Barnacles",
             "FUCEPI" = "Fucus epibiont",
             "FUCSPP" = "Fucus spp. (Rockweed)",
             "KELP"   = "Kelp",
             "MUSSPP" = "Mussels",
             "NOSAMP" = "Not sampled",
             "OTHINV" = "Other invertebrates",
             "OTHPLA" = "Other plant",
  ...
  if(!is.null(top_spp)){
    # Create df of top 4 species per community by cover
    top_df <- dat1 |> group_by(UnitCode, SiteCode, CommunityType, CoverCode) |>
    ...
    dat <- left_join(top_df, dat1,
                     by = c("UnitCode", "SiteCode", "CommunityType", "CoverCode"))

    } else if (main_groups == TRUE){
    dat <- dat1 |> dplyr::mutate(CoverCode = ifelse(CoverCode %in% c("ALGRED", "CHOMAS"), "REDGRP", CoverCode),
                                 CoverType = ifelse(CoverCode %in% "REDGRP", "Red algae group", CoverType),
                                 ScientificName = ifelse(CoverCode %in% "REDGRP", "Red algae group", ScientificName)) |>
                   dplyr::filter(CoverCode %in% c("REDGRP", "ASCNOD", "FUCSPP", "BARSPP", "MUSSPP", "NONCOR")) |>
                   group_by(UnitCode, SiteCode, Year, CommunityType, CoverCode, CoverType, ScientificName) |>
  ...
  covcode <- c("ALGBRO", "ALGGRE", "ALGRED", "ARTCOR", "ASCEPI", "ASCNOD",
               "BARSPP", "CHOMAS", "CRUCOR", "NONCOR", "FUCEPI", "FUCSPP", "KELP",
               "MUSSPP", "OTHINV", "OTHPLA", "OTHSUB", "PALPAL", "PORSPP", "ULVENT", "ULVINT",
               "ULVLAC", "UNIDEN", "BOLT", "ROCK", "SAND", "TAR", "REDGRP", "NOSAMP")

  dat$CoverCode <- factor(dat$CoverCode, levels = covcode) |> droplevels()
  spp <- levels(dat$CoverCode)
  dat$CommunityType <- factor(dat$CommunityType, levels = c("Barnacle", "Mussel", "Fucus", "Ascophyllum", "Red Algae"))

  if(main_groups == TRUE){
    dat$CoverCode <- factor(dat$CoverCode, levels = c("REDGRP", "ASCNOD", "FUCSPP", "MUSSPP", "BARSPP", "NONCOR"))
    dat$CommunityType <- factor(dat$CommunityType,
  ...
  dat2 <- dat |> group_by(UnitCode, SiteCode, CommunityType, CoverCode, CoverType, ScientificName) |>
    mutate(nz = ifelse(sum(median_cover) == 0, 0, 1))

  dat_nz <- dat2 |> filter(nz > 0) |> droplevels()
  ...
  if(heatmap == FALSE){
    ggplot(dat_nz, aes(x = Year, y = median_cover,  color = CoverCode, fill = CoverCode,
         ...
         geom_point(aes(x = Year, y = avg_cover, fill = CoverCode, size = CoverCode), color = 'black') +
         scale_shape_manual(values = shps, name = "Cover Type", breaks = names(shps),
                            labels = labels) +
         scale_size_manual(values = sz, name = "Cover Type", breaks = names(sz),
                           labels = labels) +
      ...
      ggplot(dat, aes(x = Year, y = CoverCode,  color = median_cover, fill = median_cover)) +
        ...
        {if(facet_loc_cat == TRUE) facet_wrap(~CommunityType + SiteCode,
                                              labeller = as_labeller(c(targ_labs, loc_labs)))} +
        {if(facet_loc_cat == FALSE) facet_wrap(~CommunityType, drop = T, nrow = nrow)} +
