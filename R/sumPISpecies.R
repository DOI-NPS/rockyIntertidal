...
#' site, plot name, and species. Returned values include elevation change between bolts, median elevation
#' between bolts, min/max elevation between bolts, distance between bolts, slope percent and degree between bolts,
#' horizontal distance between bolts, and interpolated elevation for point intercept sampling sites.
#'
...
sumPISpecies <- function(park = "all", site = "all", plotName = "all",
  ...
  suppressWarnings(
  boltdist <- boltdist2 |>
    arrange(UnitCode, SiteCode, StartDate, PlotName, Label) |>
    mutate(transID = paste0(SiteCode, "_", PlotName, "_", Year,
                            ifelse(QAQC == TRUE, "_Q", "")),
    ...
    group_by(SiteName, UnitCode, SiteCode, StartDate, Year, QAQC, PlotName) |>
  ...
  bolt_checks <- boltdist |> dplyr::filter(is.na(slope_rad)) |>
    select(SiteCode, Year, Label, QAQC, Elevation_MLLW_m, Distance_m) |>
    arrange(SiteCode, Year, Label, QAQC)
  ...
  sppdist <- force(getPISpecies(park = park, site = site, plotName = plotName,
                                      species = 'all', # join all spp. for geometry to work; filter spp later
                                      years = years, QAQC = QAQC)) |>
              arrange(UnitCode, SiteCode, StartDate, PlotName, PiDistance) |>
              mutate(dist_pi = PiDistance,
                     transID = paste0(SiteCode, "_", PlotName, "_", Year,
                                      ifelse(QAQC == TRUE, "_Q", "")))
  ...
  setkeyv(sppdist, c("transID", "PiDistance"))

  spp_merge1 <- boltdist[sppdist, on = c("SiteName", "UnitCode", "SiteCode", "StartDate",
                                        "Year", "QAQC", "PlotName", "transID",
                                        "dist_last" = "PiDistance"), roll = -Inf]

  suppressWarnings( #warnings covered by tryCatch above
    spp_merge <- spp_merge1 |>
    arrange(UnitCode, SiteCode, StartDate, PlotName, Label, dist_pi) |>
    mutate(dist_pi_last = dplyr::lag(dist_pi, 1, default = NA)) |>
    group_by(SiteName, UnitCode, SiteCode, StartDate, Year, QAQC, PlotName, transID, Label) |>
    ...
    select(UnitCode, SiteCode, SiteName, StartDate, QAQC, Year, PlotName,
           Label, CoverType, CoverCode, Elevation_MLLW_m, elev_first, elev_last, elev_change,
  ...
  spp_final <- if(any(species == "all")){spp_merge
    } else {spp_merge |> filter(CoverCode %in% species)}
