...
#' plotEchinoCounts(site = "CALISL", species = stars)
#' plotEchinoCounts(site = "CALISL", palette = "viridis", species = stars)
#'
#' }
#'
#'
  ...
  ggplot(dat_nz, aes(x = Year, y = count_avg, fill = SpeciesCode, color = SpeciesCode,
      ...
      {if(all(palette == 'default'))
        scale_fill_manual(values = cols, name = "Species",
                          breaks = names(cols), labels = labels)} +
      {if(all(palette == 'viridis')) 
        scale_color_viridis_d("Species", aesthetics = c("color", "fill"), labels = labels)}+
