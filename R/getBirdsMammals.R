...
#'
#' # get every observation of a dog 
#' dogs <- getBirdsMammals() |> dplyr::filter(Species == "Dog", Abundance > 0) |> 
#'   dplyr::arrange(Year)
#'
#' # matrix of wildlife detected by site 
...
getBirdsMammals <- function(park = "all", site = "all", years = 2013:as.numeric(format(Sys.Date(), "%Y"))){
  ...
  tryCatch(birdmam <- get("BirdsMammals", envir = env) |>
             dplyr::mutate(Year = as.numeric(format(as.Date(StartDate, format = "%m/%d/%Y"), "%Y"))),
