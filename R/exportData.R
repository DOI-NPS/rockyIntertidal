...
#'
#' @description This function exports data package views generated from the rocky intertidal front end
#' database individual .csv files, or as a zip file with the dataset name (NETN_RockyIntertidal_Data_Package),
#' and the date it was zipped. The exported .csv or .zip files can then be imported via importData().
#' This function is primarily for internal use to create flat files that users can import to run functions
#' in this package without having a connection having MS Access installed. Must first import the views in
...
#'
#' @return NETN rocky intertidal views exported to specified path
#'
#' @examples
#' \dontrun{
#' # RUN FIRST
...
#' exportData(filepath = "../data/rocky", zip = TRUE)
#'
#' # Export views as .csvs to specified path
#' exportData(filepath = "../data/rocky")
#'}
#'
#' @export
...
exportData <- function(filepath = NA, zip = FALSE){
  ...
  if(exists("ROCKY")){env = ROCKY} else {env = .GlobalEnv}
  ...
  if(zip == FALSE){
    ...
    zip_name = paste0("NETN_RockyIntertidal_Data_Package_", format(Sys.Date(), "%Y%m%d"), ".zip")
