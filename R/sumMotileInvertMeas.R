...
#' Species codes in the data set are: CARMAE = Carcinus maenas (green crab);
#' HEMISAN = Hemigrapsus sanguineus (Asian shore crab);
#' LITLIT = Littorina littorea (common periwinkle);
#' LITOBT = Littorina obtusata (smooth periwinkle);
#' LITSAX = Littorina saxatilis (rough periwinkle);
#' NUCLAP = Nucella lapillus (dogwhelk),
#' TECTES = Tectura testudinalis (limpet).
#'
...
sumMotileInvertMeas <- function(park = "all", site = "all", plotName = "all",
  ...
  if(any(!is.na(motinv$Measurement) & motinv$Measurement > 99.9)){
    warning("Motile Invertebrate length measured over 99.9mm. Function only built to handle lengths up to 99.9mm. If the large measurement is correct, update the measurement factor levels")}
  ...
  motinv_sum <- motinv |> group_by(GroupCode, GroupName, UnitCode, UnitName, SiteCode, SiteName, StartDate,
                                   Year, QAQC, CommunityType, ScientificName, CommonName, SpeciesCode, Meas_5mm_fac) |>
    summarize(num_meas = sum(!is.na(Measurement)), .groups = 'drop')

  return(data.frame(motinv_sum))
